...
3. 구현 + 최소 검증
   ...
   - `config/strategy.env` 튜닝 변경 시: 인벤토리 임계값/포션 재고/최소 구매 수량(`BUJU_MIN_BUY_QTY`)/골드 예비금(`BUJU_MIN_GOLD_RESERVE`)/anti-stall(`BUJU_STALL_*`)/재시도 상한(`BUJU_RETRY_MAX_ATTEMPTS`) 의도와 README Live Strategy Runner 정책 설명을 함께 동기화
