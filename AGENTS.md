...
3. 구현 + 최소 검증
  ...
  - README의 "현재 기본값" 표기(특히 `BUJU_BASE_DELAY_MS`, `BUJU_MAX_ACTIONS_PER_CYCLE`, `BUJU_MIN_GOLD_RESERVE`, `BUJU_MUTATION_MIN_GOLD_RESERVE`, `BUJU_STALL_429_COOLDOWN_TICKS`, `BUJU_COMBAT_STRATEGY_REFRESH_TICKS`, `BUJU_BACKOFF_BASE_MS`)가 `config/strategy.env` 실값과 일치하는지 함께 확인
