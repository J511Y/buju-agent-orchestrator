...
3. 구현 + 최소 검증
   ...
   - Live runner(`scripts/live-action-tick.js`, `scripts/live-strategy-runner.js`, `scripts/live-runner-daemon.sh`) 변경 시: dry-run 출력 확인 + 로그 경로(`logs/live-runner-daemon.log`) 및 전투 진입 계약(`combat/strategy` 갱신 + `combat/start`의 `monster_id`,`area` + 필요 시 `hunt` 폴백 `monster_id`,`skill_id`) 유지 확인
   ...
   - `config/strategy.env` 튜닝 변경 시: 기본 페이싱(`BUJU_BASE_DELAY_MS`)/사이클 쿼터(`BUJU_MAX_ACTIONS_PER_CYCLE`)/인벤토리 임계값/포션 재고/최소 구매 수량(`BUJU_MIN_BUY_QTY`)/골드 예비금(`BUJU_MIN_GOLD_RESERVE`)/anti-stall(`BUJU_STALL_*`)/재시도 상한(`BUJU_RETRY_MAX_ATTEMPTS`)/전투 진입 토글(`BUJU_USE_COMBAT_START`)/강화 안전장치(`BUJU_ENHANCE_*`) 의도와 README Live Strategy Runner 정책 설명을 함께 동기화
