...
- 현재 우선순위 정책:
  ...
  - `BUJU_STALL_429_COOLDOWN_TICKS`/`BUJU_RETRY_MAX_ATTEMPTS`/`BUJU_BACKOFF_*` 조합으로 429 루프를 냉각하며, 반복 구간에서는 액션 빈도를 낮춰 재진입(현재 기본값 예: `BUJU_STALL_429_COOLDOWN_TICKS=30`, `BUJU_COMBAT_STRATEGY_REFRESH_TICKS=40`, `BUJU_BACKOFF_BASE_MS=1500`)
