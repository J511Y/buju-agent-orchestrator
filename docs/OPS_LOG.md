...
- [2026-03-18 15:48 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: live monster/status probes were not refreshable this cycle due DNS failure, so safest high-efficiency routing stayed pinned to last validated baseline (`skeleton` in `talking_island_field`) with strict movement threshold gate unchanged (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated by policy: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active; staged enhancement strategy remains explicit in `docs/DECISIONS.md`; minimal safe enhancement path remained prerequisite-gated this cycle because `scroll+npc+resource` satisfiability probe could not be refreshed.
  - Live continuity preserved: daemon lineage remained active (`tmp/live-runner-procs-1548.txt`) with no intentional stop/restart action.
  - Ops telemetry post attempted with concrete delta reasoning (`tmp/thinking-post-1553.json`) and failed on the same DNS path (`tmp/thinking-post-response-1553.meta`: `000`, `curl_exit=6`; stderr in `tmp/thinking-post-response-1553.err`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=3/20`, and progression `exp>=1660` or `gold>=444` with daemon continuity.
...
- [2026-03-18 11:22 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`tmp/cron-status-now-1121.json`) + `GET /api/areas/talking_island_field/monsters => 200` (`tmp/cron-monsters-now-1121.json`) keep safest high-efficiency target as `skeleton` (same risk band, higher EXP than `rabbit`); strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active (`tmp/cron-inventory-now-1121.json`); staged enhancement strategy remains explicit in `docs/DECISIONS.md`; minimal safe enhancement path stayed prerequisite-gated this cycle (`tmp/cron-npc-now-1121.json` with `npcs=[]`).
  - Live continuity preserved: smoke succeeded (`tmp/cron-smoke-1122.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=28`, `exp=6529`, `gold=429`, `code=200`) and daemon lineage remained active (`tmp/live-runner-procs-1122.txt`) with no intentional stop/restart action.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=4/20 mentions`, and progression `exp>=6600` or `gold>=444` with daemon continuity.

- [2026-03-18 10:48 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - CHANGE evidence (mandatory adaptive loop): `GET /api/agent/thinking/j211y?limit=20` returned empty (`tmp/cron-thinking-now-1048.json`, `count=0`), so ordered local fallback deltas were recomputed (`tmp/cron-last20-delta-1048-local.json`) and remained non-improving for KEEP (`level +1`, `gold +5`, `inventory +4`, `exp -5793`, `death/defeat mentions 20`, `rate/429/cooldown mentions 20`).
  - Minimal reversible CHANGE applied: `config/strategy.env` tuned `BUJU_BACKOFF_BASE_MS 2400->2500` (README current-default note synced) to damp immediate retry re-entry under persistent cooldown/rate-limit pressure.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`tmp/cron-status-now-1048.json`) + `GET /api/areas/talking_island_field/monsters => 200` (`tmp/cron-monsters-now-1048.json`, scored in `tmp/cron-monster-score-1048.json`) keep safest high-efficiency target as `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active (`tmp/cron-inventory-now-1048.json`); staged enhancement strategy remains explicit in `docs/DECISIONS.md`; minimal safe enhancement path stayed prerequisite-gated this cycle (`tmp/cron-npc-now-1048.json` with `npcs=[]`).
  - Live continuity preserved: smoke succeeded (`tmp/cron-smoke-1048.txt`) and daemon lineage remained active (`tmp/live-runner-procs-1048.txt`) with no intentional stop/restart action.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=5/20 mentions`, and progression `exp>=6220` or `gold>=439` with daemon continuity.
...
- [2026-03-18 10:18 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`tmp/cron-status-now-1018.json`) and `GET /api/areas/talking_island_field/monsters => 200` (`tmp/cron-monsters-now-1018.json`) keep safest high-efficiency target as `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active (`tmp/cron-inventory-now-1018.json`); staged enhancement strategy remains explicit in `docs/DECISIONS.md`; minimal safe enhancement path stayed prerequisite-gated this cycle (`tmp/cron-npc-now-1018.json` with `npcs=[]`).
  - Live continuity preserved: smoke succeeded after change (`tmp/cron-smoke-1018.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=28`, `exp=5801`, `gold=439`, `code=200`) and daemon lineage remained active (`tmp/live-runner-procs-1018.txt`) with no intentional stop/restart action.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=5/20 mentions`, and progression `exp>=5870` or `gold>=444` with daemon continuity.
...
- [2026-03-18 06:50 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`tmp/cron-status-now-0650.json`) and `GET /api/areas/talking_island_field/monsters => 200` (`tmp/cron-monsters-now-0650.json`) keep safest high-efficiency target as `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active (`tmp/cron-inventory-now-0650.json`); staged enhancement strategy remains explicit in `docs/DECISIONS.md`; minimal safe enhancement path stayed prerequisite-gated this cycle (`tmp/cron-npc-now-0650.json` with `npcs=[]`).
  - Live continuity preserved: smoke succeeded (`tmp/cron-smoke-0650.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=28`, `exp=3277`, `gold=439`, `code=200`) and daemon lineage remained active (`tmp/live-runner-procs-0650.txt`) with no stop/restart action.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=7/20 mentions`, and progression `exp>=3350` or `gold>=444` with daemon continuity.

- [2026-03-18 06:19 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - CHANGE evidence (mandatory adaptive loop): remote `GET /api/agent/thinking/j211y?limit=20` plus live status/monster/inventory/npc probes DNS-failed (`tmp/cron-thinking-now-0619.json`, `tmp/cron-status-now-0619.json`, `tmp/cron-monsters-now-0619.json`, `tmp/cron-inventory-now-0619.json`, `tmp/cron-npc-now-0619.json`), so ordered local fallback deltas were recomputed (`tmp/cron-last20-delta-0619-local.json`) and remained non-improving (`level +1`, `gold +10`, `inventory +5`, `death/defeat mentions 20`, `rate/429/cooldown mentions 20`; `exp` unavailable in thinking context).
  - Minimal reversible tuning applied: `config/strategy.env` set `BUJU_STALL_429_COOLDOWN_TICKS 48->50` (README current-default note synced) to widen post-429 cooling and reduce immediate throttle re-entry while preserving strict safety/movement/equipment invariants.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  ...
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active; staged enhancement strategy remains explicit in `docs/DECISIONS.md`; minimal safe enhancement path stayed prerequisite-gated this cycle.
  - Live continuity preserved: smoke succeeded (`tmp/cron-smoke-0619.txt`: `ok=1/1`, `lastAction=combat_start`, `level=28`, `exp=2915`, `gold=444`, `code=200`) and daemon lineage remained active (`tmp/live-runner-procs-0619.txt`) with no stop/restart action.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=7/20 mentions`, and progression `exp>=2990` or `gold>=449` with daemon continuity.
...
- [2026-03-18 05:18 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Minimal reversible tuning applied: `config/strategy.env` set `BUJU_STALL_429_COOLDOWN_TICKS 44->46` (README current-default note synced) to widen post-429 cooling and reduce immediate re-entry churn while preserving strict safety/movement/equipment invariants.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`tmp/cron-status-now-0518.json`) and `GET /api/areas/talking_island_field/monsters => 200` (`tmp/cron-monsters-now-0518.json`) keep safest high-efficiency target as `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active (`tmp/cron-inventory-now-0518.json`), staged enhancement strategy remains explicit in `docs/DECISIONS.md`, and minimal safe enhancement path stayed prerequisite-gated this cycle (`tmp/cron-npc-now-0518.json` with `npcs=[]`).
  - Live continuity preserved: smoke succeeded (`tmp/cron-smoke-0518.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=28`, `exp=2197`, `gold=439`, `code=200`) and daemon lineage remained active (`tmp/live-runner-procs-0518.txt`) with no stop/restart action.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=8/20 mentions`, and progression `exp>=2270` or `gold>=449` with daemon continuity.

- [2026-03-18 04:50 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - CHANGE evidence (mandatory adaptive loop): `GET /api/agent/thinking/j211y?limit=20` returned ordered `count=20` (`tmp/cron-thinking-now-0450.json`), and ordered delta was mixed/non-improving (`tmp/cron-last20-delta-0450.json`: `level +2`, `gold -10`, `inventory +4`, `death/defeat mentions 15`, `rate/429/cooldown mentions 13`), so KEEP was rejected.
  - Minimal reversible tuning applied: `config/strategy.env` set `BUJU_BACKOFF_BASE_MS 2100->2200` (README default note synced) to lower immediate retry re-entry under cooldown/rate-limit wording while preserving strict safety/movement/equipment invariants.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`tmp/cron-status-now-0450.json`) and `GET /api/areas/talking_island_field/monsters => 200` (`tmp/cron-monsters-now-0450.json`) keep safest high-efficiency target as `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active (`tmp/cron-inventory-now-0450.json`); staged enhancement strategy remains explicit in `docs/DECISIONS.md`; minimal safe enhancement path stayed prerequisite-gated this cycle (`tmp/cron-npc-now-0450.json`).
  - Live continuity preserved: smoke succeeded (`tmp/cron-smoke-0450.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=28`, `exp=1845`, `gold=439`, `code=200`) and daemon lineage remained active (`tmp/live-runner-procs-0450.txt`) with no stop/restart action.
  - Ops telemetry posted: `POST /api/agent/thinking => 200 {"success":true}` (`tmp/thinking-post-0450.json`, `tmp/thinking-post-response-0450.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=9/20`, and progression `exp>=1910` or `gold>=444` with daemon continuity.
...
- [2026-03-18 04:18 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`tmp/cron-status-now-0418.json`) and `GET /api/areas/talking_island_field/monsters => 200` (`tmp/cron-monsters-now-0418.json`) keep safest high-efficiency target as `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active (`tmp/cron-inventory-now-0418.json`); staged enhancement strategy remains explicit in `docs/DECISIONS.md`; minimal safe enhancement path stayed prerequisite-gated this cycle (`tmp/cron-npc-now-0418.json`).
  - Live continuity preserved: smoke succeeded (`tmp/cron-smoke-0418.txt`: `ok=1/1`, `lastAction=combat_start`, `level=28`, `exp=1467`, `gold=444`, `code=200`) and daemon lineage remained active (`tmp/live-runner-procs-0418.txt`) with no stop/restart action.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=10/20 mentions`, and progression `exp>=1540` or `gold>=449` with daemon continuity.
...
- [2026-03-17 23:48 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=27`, `exp=5435`, `gold=439`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`), so safest high-efficiency target remains `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active; staged enhancement strategy remains explicit in `docs/DECISIONS.md`; minimal safe enhancement path stayed prerequisite-gated (`GET /api/npc/list => 200`, `npcs=[]`).
  - Live continuity preserved: smoke succeeded (`tmp/cron-smoke-2348.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=27`, `exp=5441`, `gold=444`, `code=200`) and daemon lineage remained active (`tmp/live-runner-procs-2348.txt`) with no stop/restart action.
  - Ops telemetry posted: `POST /api/agent/thinking => 200 {"success":true}` (`tmp/thinking-post-2348.json`, `tmp/thinking-post-response-2348.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `surrender_dangerous_combat<=1/20`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=10/20`, and progression `exp>=5510` or `gold>=449` with daemon continuity.
...
- [2026-03-17 19:49 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=27`, `exp=2501`, `gold=434`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`), so safest high-efficiency target remains `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active; staged enhancement strategy remains explicit in `docs/DECISIONS.md` (early gold-safe, mid weapon-first on reserve/prereqs, late armor/accessory with failure-risk controls); minimal safe enhancement path stayed prerequisite-gated this cycle (`GET /api/npc/list => 200`, `npcs=[]`).
  - Live continuity preserved: smoke after change succeeded (`tmp/cron-smoke-1949.txt`: `verify:cycle passed`) and daemon lineage remained active (`tmp/live-runner-procs-1949.txt`), with no stop/restart action.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=8/20`, and progression `exp>=2570` or `gold>=444` with daemon continuity.
...
- [2026-03-17 15:51 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=26`, `exp=6383`, `gold=439`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`), so safest high-efficiency target remains `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active, staged enhancement policy remains explicit in `docs/DECISIONS.md`, and minimal safe enhancement path stayed prerequisite-gated this cycle (`GET /api/npc/list => 200`, `npcs=[]`, no scroll).
  - Live continuity preserved: smoke after change succeeded (`tmp/cron-smoke-1551.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=26`, `exp=6397`, `gold=444`, `code=200`), daemon lock-owner remained active (`logs/live-runner-daemon.lock`), and telemetry posted (`tmp/thinking-post-response-1551.json` => `{"success":true}`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `rate/cooldown mentions<=12/20`, and progression `exp>=6460` or `gold>=449` with daemon continuity.
...
- [2026-03-17 13:48 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=26`, `exp=5015`, `gold=439`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`), so safest high-efficiency target remains `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active (`short_sword`; `tmp/cron-inventory-now-1348.json`), staged enhancement policy remains explicit in `docs/DECISIONS.md`, and minimal safe enhancement path stayed prerequisite-gated this cycle (`tmp/cron-npc-now-1348.json` => `npcCount=0`, no scroll).
  - Live continuity preserved: smoke after change succeeded (`tmp/cron-smoke-1348.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=26`, `exp=5027`, `gold=439`, `code=200`), daemon lineage remained active (`tmp/live-runner-procs-1348.txt`), and telemetry posted (`tmp/thinking-post-response-1348.json` => `{"success":true}`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `rate/cooldown mentions<=12/20`, and progression `exp>=5100` or `gold>=449` with daemon continuity.
...
- [2026-03-17 07:19 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=26`, `exp=2285`, `gold=424`, `area=talking_island_field`, `in_combat=false`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`), so safest high-efficiency target remains `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active; staged enhancement plan remains in `docs/DECISIONS.md`; minimal safe enhancement path remains prerequisite-gated this cycle (`GET /api/npc/list => 200`, `npcs=[]`, enchant scroll stock `0`).
  - Live continuity evidence: daemon lineage remains active (`tmp/live-runner-procs-0719.txt`); smoke probe recorded (`tmp/cron-smoke-0719.txt`: `ok=0/1`, `lastAction=hunt`, `code=400`) with no daemon restart.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, low-HP stall mentions `<=6/20`, and progression `exp>=2350` or `gold>=434`.
...
- [2026-03-17 05:18 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=26`, `exp=2277`, `gold=434`, `area=talking_island_field`), `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`), so safest high-efficiency target remains `skeleton`; strict movement threshold gate remains (`BUJU_MOVE_LEVEL_2=30`).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active (`short_sword` > `rusty_sword`); staged enhancement plan in `docs/DECISIONS.md` remains explicit; minimal safe enhancement path remains prerequisite-gated this cycle (`GET /api/npc/list => 200`, `npcs=[]`).
  - Live continuity evidence: daemon lineage remains active (`tmp/live-runner-procs-0518.txt`); strategy smoke passed after change (`tmp/cron-smoke-0518.txt`: `ok=1/1`, `lastAction=buy_mp`, `level=26`, `exp=2285`, `gold=444`, `code=200`).
  - Ops telemetry posted: `POST /api/agent/thinking => 200 {"success":true}` (`tmp/thinking-post-0518.json`, `tmp/thinking-post-response-0518.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=8%`, and progression `exp>=2360` or `gold>=454`.

- [2026-03-17 04:48 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` returned `count=20`; ordered window (`2026-03-16 18:50:43 -> 2026-03-17 04:20:38`) improved (`level +1`, `gold +40`, `inventory +0`, `rate/429/cooldown mentions 10/20`), so CHANGE was not applied.
  - Safety/efficiency evidence: direct read-path probes were auth-blocked (`GET /api/status => 401`, `GET /api/npc/list => 401`), so live area/level/gold context was sourced from latest thinking context (`level=26`, `area=talking_island_field`, `gold=444`); `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`) keeps safest high-efficiency target as `skeleton`, with strict movement gate (`BUJU_MOVE_LEVEL_2=30`) retained to avoid repeated defeats.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) remains active by policy; staged enhancement plan in `docs/DECISIONS.md` remains explicit (early safe-gold/no spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated and was skipped due unsatisfied auth/prereq visibility this cycle.
  - Live continuity evidence: daemon lineage remains active (`tmp/cron-procs-0448.txt`); strategy smoke passed (`tmp/cron-smoke-0448.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=26`, `exp=1935`, `gold=439`, `code=200`).
  - Ops telemetry post attempt: `POST /api/agent/thinking => 401 UNAUTHORIZED`; payload/response archived (`tmp/thinking-post-0448.json`, `tmp/thinking-post-response-0448.json`) for replay after auth recovery.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=11%`, and progression `exp>=2020` or `gold>=449`.
...
- [2026-03-17 03:18 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) active (`short_sword` over `rusty_sword`); staged enhancement policy in `docs/DECISIONS.md` remains explicit (early safe-gold/no spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`GET /api/npc/list => 200`, `npcs=[]`, enchant-scroll stock `0`).
  - Ops telemetry posted: first `POST /api/agent/thinking` failed validation (`reasoning <= 500 chars`), corrected payload then succeeded (`tmp/thinking-post-0318.json`, `tmp/thinking-post-response-0318.json` => `{"success":true}`).
  - Live continuity evidence: daemon lock-owner lineage remains active (`tmp/cron-0318-procs.txt`, lock pid `72490`); strategy smoke passed (`tmp/cron-smoke-0318.txt`: `ok=1/1`, `lastAction=combat_start`, `level=26`, `exp=945`, `gold=444`, `code=200`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=11%`, and progression `exp>=1030` or `gold>=454`.
...
- [2026-03-17 02:19 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) active (`short_sword` over `rusty_sword`); staged enhancement policy in `docs/DECISIONS.md` remains explicit (early safe-gold/no spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`GET /api/npc/list => 200`, `npcs=[]`, enchant-scroll stock `0`).
  - Ops telemetry posted: `POST /api/agent/thinking => 200 {"success":true}` (`tmp/thinking-post-0219.json`, `tmp/thinking-post-response-0219.json`).
  - Live continuity evidence: daemon lineage remains active (`tmp/live-runner-procs-0219.txt`); strategy smoke passed (`tmp/cron-smoke-0219.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=26`, `exp=261`, `gold=434`, `code=200`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=11%`, and progression `exp>=340` or `gold>=454`.

- [2026-03-17 01:49 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): remote `GET /api/agent/thinking/j211y?limit=20` returned `count=20`; ordered window (`2026-03-16 15:50:10 -> 2026-03-17 01:20:26`) improved (`level +0`, `exp +0`, `gold +30`, `inventory -3`) with latest death unchanged (`2026-03-15 05:09:30`), so CHANGE was not applied.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=25`, `exp=6157`, `gold=439`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton`, movement threshold gate remains strict (`BUJU_MOVE_LEVEL_2=30`).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) active (`short_sword` over `rusty_sword`); staged enhancement policy in `docs/DECISIONS.md` remains explicit (early safe-gold/no spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`GET /api/npc/list => 200`, `npcs=[]`, enchant-scroll stock `0`).
  - Ops telemetry posted: `POST /api/agent/thinking => 200 {"success":true}` (`tmp/thinking-post-0149.json`, `tmp/thinking-post-response-0149.json`).
  - Live continuity evidence: daemon lineage remains active (`tmp/live-runner-procs-0149.txt`); strategy smoke passed (`tmp/cron-smoke-0149.txt`: `ok=1/1`, `lastAction=combat_start`, `level=25`, `exp=6161`, `gold=439`, `code=200`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=11%`, and progression `exp>=6245` or `gold>=449`.
...
- [2026-03-16 23:18 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) active; staged enhancement policy in `docs/DECISIONS.md` remains explicit (early safe-gold/no spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`GET /api/npc/list => 200`, `npcs=[]`, enchant-scroll stock `0`).
  - Ops telemetry posted: `POST /api/agent/thinking => 200 {"success":true}` (`tmp/thinking-post-2318.json`, `tmp/thinking-post-response-2318.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=11%`, and progression `exp>=4560` or `gold>=424`.

- [2026-03-16 22:48 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` returned empty (`count=0`), so fallback local last-20 posts were delta-scored (`tmp/cron-last20-delta-local-2248.json`, `tmp/thinking-post-1147.json -> tmp/thinking-post-2218.json`, `count=20`) and still improved (`level +1`, `gold +20`, `inventory +1`, `rate/429/cooldown mentions 15/20`), so CHANGE was not applied.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=25`, `exp=4217`, `gold=419`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton`, movement threshold gate remains strict (`BUJU_MOVE_LEVEL_2=30`).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) active; staged enhancement policy remains explicit (early safe-gold/no spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`GET /api/npc/list => 200`, `npcs=[]`, enchant-scroll stock `0`).
  - Live continuity evidence: daemon remains continuous (`tmp/live-runner-procs-2248.txt`: `live-runner-daemon.sh` + `live-strategy-runner.js`); smoke passed (`tmp/cron-smoke-2248.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `code=200`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=11%`, and progression `exp>=4300` or `gold>=429`.
...
- [2026-03-16 20:26 KST] Hourly Buju gameplay-feedback cycle executed.
  ...
  - Last-hour gameplay signals: progression `unknown/blocked` (collector fallback zero-delta conflicts with direct auth failure), wins/defeats `unresolved`, resource trend `unresolved`, anomaly=`read-path auth divergence` (`collector status=200` vs direct `401`).
  - Development feedback: treat this hour as telemetry-integrity incident, not gameplay regression; block policy/aggression tuning until a canonical auth preflight returns `200` for both status and logs on the same client path.
  - Failure mode + retry recommendation: rerun masked preflight (`/api/status` + `/api/logs?limit=1`) with 2 attempts and 5-10s jittered backoff; if either probe stays non-200, classify `auth_blocked` and skip gameplay inference.
  - Next 30m actionable TODO: implement `tmp/hourly-auth-parity.json` emission in hourly flow (`collector_status_http`, `direct_status_http`, `direct_logs_http`, `inference_allowed`) and gate doc synthesis on `inference_allowed=true`.

- [2026-03-16 20:18 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` returned `count=20`; ordered window delta (`2026-03-16 09:49:44 -> 19:51:31`) was mixed-positive (`level +1`, `inventory -1`, `death delta +0`, `gold -15`) with throttle wording `15/20`, so CHANGE was not applied.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=25`, `exp=2895`, `gold=414`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton`, movement threshold gate remains strict (`BUJU_MOVE_LEVEL_2=30`).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) active; staged enhancement plan remains explicit in `docs/DECISIONS.md` (early safe-gold/no spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`GET /api/npc/list => 200`, `npcs=[]`, enchant-scroll stock `0`).
  - Live continuity evidence: daemon remains continuous (`tmp/live-runner-procs-2018.txt`: `live-runner-daemon.sh` pid `5854`); smoke passed (`tmp/cron-smoke-2018.txt`: `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `code=200`).
  - Ops telemetry posted: `POST /api/agent/thinking => 200 {"success":true}` (`tmp/thinking-post-2018.json`, `tmp/thinking-post-response-2018.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=13%`, and progression `exp>=2980` or `gold>=428`.

- [2026-03-16 19:48 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` returned `count=20`; ordered window delta (`2026-03-16 09:19:24 -> 19:19:55`) was positive (`level +1`, `gold +25`, `inventory +1`, death-log delta `+0`), so CHANGE was not applied.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=25`, `exp=2621`, `gold=409`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton`, movement threshold gate remains strict (`BUJU_MOVE_LEVEL_2=30`).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) active; staged enhancement plan remains explicit in `docs/DECISIONS.md` (early safe-gold/no spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`GET /api/npc/list => 200`, `npcs=[]`, enchant-scroll stock `0`).
  - Live continuity action: duplicate stale daemon lineage was cleaned and single lock-owner daemon retained (`logs/live-runner-daemon.lock` PID `5854`); runtime snapshot archived (`tmp/live-runner-procs-1948.txt`).
  - Live smoke: `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=25`, `exp=2627`, `gold=404`, `code=200` (`tmp/cron-smoke-1948.txt`).
  - Ops telemetry posted: `POST /api/agent/thinking => 200 {"success":true}` (`tmp/thinking-post-1948.json`, `tmp/thinking-post-response-1948.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=14%`, and progression `exp>=2700` or `gold>=430`.
...
- [2026-03-16 15:49 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=25`, `exp=483`, `gold=414`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton`, movement threshold gate remains strict (`BUJU_MOVE_LEVEL_2=30`).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; at `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip (`equipSlot + score(maxDamage+defBonus)`) active; staged enhancement plan remains explicit in `docs/DECISIONS.md` (early safe gold/no spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`GET /api/npc/list => 200`, `npcs=[]`, enchant-scroll stock `0`).
  - Runtime continuity evidence: daemon lock PID alive (`logs/live-runner-daemon.lock` => `20502`), and `logs/live-runner-daemon.log` continued appending through this run.
  - Ops telemetry posted: first post attempt failed validation (`400 tick_number required`), corrected payload then succeeded (`POST /api/agent/thinking` => `200 {"success":true}`; artifacts `tmp/thinking-post-1549.json`, `tmp/thinking-post-response-1549.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=14%`, and progression `exp>=760` or `gold>=430`.
...
- [2026-03-16 13:17 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=24`, `exp=4901`, `gold=409`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton` with strict movement threshold gate (`BUJU_MOVE_LEVEL_2=30`).
  ...
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=24`, `exp=4907`, `gold=414`, `code=200` (`tmp/cron-smoke-1317.txt`).
  - Runtime continuity evidence: daemon remains continuous (`live-runner-daemon.sh` + `live-strategy-runner.js` active; `tmp/live-runner-procs-1317.txt`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-response-1317.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=15%`, and progression `exp>=4980` or `gold>=426` with smoke `code=200`.
...
- [2026-03-16 11:19 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip by `equipSlot + score(maxDamage+defBonus)` remains active; staged enhancement plan in `docs/DECISIONS.md` remains explicit (early no-risky-spam -> mid weapon-first with reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains implemented and prerequisite-gated (`GET /api/npc/list => npcs=[]`, no enchant scroll).
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=combat_start`, `level=24`, `exp=3839`, `gold=394`, `code=200` (`tmp/cron-smoke-1119.txt`).
  - Runtime continuity evidence: daemon remains continuous (`live-runner-daemon.sh` + `live-strategy-runner.js` active; `tmp/live-runner-procs-1119.txt`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-response-1119.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=18%`, and progression `exp>=3910` or `gold>=405` with smoke `code=200`.
...
- [2026-03-16 05:17 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=24`, `exp=1127`, `gold=364`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton`, and movement remains threshold-gated (`BUJU_MOVE_LEVEL_2=30`).
  - Defeat trend evidence: `GET /api/logs?action=death&limit=100 => 200`, latest death timestamp still `2026-03-15 05:09:30` (no fresh defeat burst).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip by `equipSlot + score(maxDamage+defBonus)` remains active; staged enhancement plan in `docs/DECISIONS.md` remains explicit (early no-risky-spam -> mid weapon-first with reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains implemented and prerequisite-gated (`GET /api/npc/list => npcs=[]`, enchant scroll count `0`).
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=combat_start`, `level=24`, `exp=1135`, `gold=374`, `code=200` (`tmp/cron-smoke-0517.txt`).
  - Runtime continuity evidence: daemon remains continuous (`live-runner-daemon.sh` + `live-strategy-runner.js` active; `tmp/live-runner-procs-0517.txt`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-response-0518.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=22%`, and progression `exp>=1210` or `gold>=390` with smoke `code=200`.

- [2026-03-16 04:47 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` ordered window (`2026-03-15 17:50:30 -> 2026-03-16 04:20:16`, `count=20`) yielded `level +1`, `exp sparse/0`, `gold +5`, `inventory -2`, `rate/cooldown mentions 12/20`; concrete improvement evidence exists, so CHANGE was not applied.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=24`, `exp=899`, `gold=364`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton`, with strict movement threshold gate (`BUJU_MOVE_LEVEL_2=30`) and risk-gap controls preserved.
  - Defeat trend evidence: `GET /api/logs?action=death&limit=100 => 200`, latest death timestamp remains `2026-03-15 05:09:30` (no fresh defeat burst in current window).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip by `equipSlot + score(maxDamage+defBonus)` remains active; staged enhancement plan in `docs/DECISIONS.md` remains explicit (early no-risky-spam -> mid weapon-first with reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains implemented and prerequisite-gated (`GET /api/npc/list => npcs=[]`, enchant scroll count `0`).
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=buy_hp`, `level=24`, `exp=903`, `gold=374`, `code=200` (`tmp/cron-smoke-0447.txt`).
  - Runtime continuity evidence: daemon remains continuous (`live-runner-daemon.sh` + `live-strategy-runner.js` active; `tmp/live-runner-procs-0447.txt`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-response-0447.json`; first attempt rejected due reasoning length, corrected and reposted).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=24%`, and progression `exp>=980` or `gold>=390` with smoke `code=200`.

- [2026-03-16 04:17 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` ordered window (`2026-03-15 17:19:05 -> 2026-03-16 03:50:10`, `count=20`) yielded `level +1`, `exp +0`, `gold +5`, `inventory -1`, `rate/cooldown mentions 17/20`; concrete improvement evidence exists, so CHANGE was not applied.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=24`, `exp=667`, `gold=364`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton`, with strict movement threshold gate (`BUJU_MOVE_LEVEL_2=30`) and no-armor retreat guard preserved.
  - Defeat trend evidence: `GET /api/logs?action=death&limit=100 => 200` (`count=100`), latest death timestamp still `2026-03-15 05:09:30` (no fresh death burst in current decision window).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip by `equipSlot + score(maxDamage+defBonus)` remains active; staged enhancement plan in `docs/DECISIONS.md` remains explicit (early no-risky-spam -> mid weapon-first with reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains implemented and prerequisite-gated (`GET /api/npc/list => npcs=[]`, enchant scroll count `0`).
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=combat_start`, `level=24`, `exp=671`, `gold=374`, `code=200` (`tmp/cron-smoke-0417.txt`).
  - Runtime continuity evidence: daemon remains continuous (`live-runner-daemon.sh` + `live-strategy-runner.js` active; `tmp/live-runner-procs-0417.txt`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-response-0417.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=24%`, and progression `exp>=760` or `gold>=390` with smoke `code=200`.
...
- [2026-03-16 00:50 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=23`, `exp=4295`, `gold=349`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton` and movement remains threshold-gated (`BUJU_MOVE_LEVEL_2=30`) to avoid repeated defeats.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip by `equipSlot + score(maxDamage+defBonus)` remains active; staged enhancement plan in `docs/DECISIONS.md` is explicit (early no-risky-spam -> mid weapon-first with reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains implemented and prerequisite-gated (`GET /api/npc/list => npcs=[]`, enchant scroll count `0`).
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=combat_start`, `level=23`, `exp=4309`, `gold=354`, `code=200` (`tmp/cron-smoke-0050.txt`).
  - Runtime continuity evidence: daemon remains continuous (`live-runner-daemon.sh` + `live-strategy-runner.js` active; `tmp/live-runner-procs-0050.txt`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-response-0050.json`).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `buy actions<=2` per short window, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=26%`, and progression `exp>=4380` or `gold>=366` with smoke `code=200`.

## 2026-03-15
- [2026-03-15 23:47 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - CHANGE (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` ordered window (`2026-03-15 12:49:18 -> 23:19:29`, `count=20`) yielded mixed deltas (`level +1`, `gold -5`, `inventory +1`, `exp sparse`, `rate/cooldown mentions 14/20`), so KEEP was rejected on economy drift.
  - Minimal/reversible CHANGE committed: `config/strategy.env` tuned `BUJU_MIN_HP_POTION_S 6->5` to reduce buy churn while preserving HP safety, hard inventory invariants, and threshold movement gates.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=23`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` (`rabbit`,`skeleton`); safest high-efficiency target remains `skeleton` and movement remains threshold-gated (`BUJU_MOVE_LEVEL_2=30`) to avoid repeated defeats.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  ...
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=23`, `exp=3813`, `gold=344`, `code=200` (`tmp/cron-smoke-2347.txt`).
  - Runtime continuity evidence: daemon remains continuous (`live-runner-daemon.sh` + `live-strategy-runner.js` active; `tmp/live-runner-procs-2347.txt`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-response-2347.json`).
  - Next 30m KPI: `death delta=0`, inventory `<=8`, `buy_hp<=1` per short window, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=28%`, and progression `exp +>=70` or `gold +>=12` with smoke `code=200`.
...
- [2026-03-15 21:48 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip by `equipSlot + score(maxDamage+defBonus)` remains active; staged enhancement plan remains in `docs/DECISIONS.md`; minimal safe enhancement path remains implemented and safely skipped (`GET /api/npc/list => npcs=[]`, enchant scroll counts `0`).
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=23`, `exp=2839`, `gold=359`, `code=200` (`tmp/cron-smoke-2148.txt`).
  - Runtime continuity evidence: daemon remains continuous (`live-runner-daemon.sh` + `live-strategy-runner.js` active; `tmp/live-runner-procs-2148.txt`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-response-2148.json`).
  - Next 30m KPI: `death delta=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=28%`, and progression `exp +>=70` or `gold +>=12` with smoke `code=200`.
...
- [2026-03-15 19:26 KST] Hourly gameplay-feedback cycle (Buju API live probe).
  ...
  - Last-hour gameplay signals: progression, wins/defeats, and resource trends are unavailable for this window due to read-auth failure (insufficient evidence for gameplay inference).
  - Anomaly: simultaneous auth failure on status+logs indicates credential invalidation/scope drift or key-source mismatch rather than gameplay behavior change.
  - Dev feedback: freeze gameplay-policy tuning for this hour; prioritize auth-path restoration and deterministic preflight before ingesting KPI deltas.
  - Failure mode + retry recommendation: next cycle run masked preflight (`/api/status` + `/api/logs?limit=1`) with identical env loader, 2 attempts with 5-10s jittered backoff; if still non-200 classify `auth_blocked` and skip synthesis.
  - Next 30m actionable TODO: implement `scripts/auth-preflight-gate-v2.js` that emits `ok|unauthorized|source_mismatch|expired_key` and hard-blocks hourly summaries unless state=`ok`.

- [2026-03-15 18:46 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` ordered window (`08:49:44 -> 18:19:31`, `count=20`) produced `level +1`, `gold +0`, `exp sparse`, `inventory +3`, `rate/cooldown mentions 16/20`; improvement evidence exists via level gain, so CHANGE was not applied.
  - Safety/efficiency evidence: `GET /api/status => 200` (`level=23`, `area=talking_island_field`) and `GET /api/areas/talking_island_field/monsters => 200` keep safest high-efficiency target as `skeleton`; movement remains threshold-gated (`BUJU_MOVE_LEVEL_2=30`) to avoid repeated defeat risk.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation remains unequipped-worse-than-equipped first.
  - Equipment progression revalidated: BiS auto-equip by `equipSlot + score(maxDamage+defBonus)` remains active; staged enhancement plan stays in `docs/DECISIONS.md`; minimal safe enhancement path remains implemented and safely skipped (`GET /api/npc/list => npcs=[]`, enchant scroll counts `0`).
  - Live evidence: `BUJU_MAX_ACTIONS_PER_CYCLE=3 node scripts/live-strategy-runner.js` returned `live-strategy ok=1/3 lastAction=wait_combat_start_cooldown level=23 exp=1399 gold=349 code=400` (non-fatal cooldown sample); daemon continuity confirmed by `pgrep` (`live-runner-daemon.sh`, `live-strategy-runner.js` both active).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}`.
  - Next 30m KPI: `death delta=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=30%`, and progression `exp +>=60` or `gold +>=10` with at least one smoke `code=200`.
...
- [2026-03-15 10:46 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Safety/efficiency evidence now: `GET /api/status` => `200` (`level=22`, `exp=2707`, `gold=349`, `area=talking_island_field`) and current monsters remain low-risk/high-efficiency (`rabbit`, `skeleton`), so safest target remains `skeleton`; movement gate remains strict (`BUJU_MOVE_LEVEL_2=30`) to avoid repeated defeats.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation still prioritizes selling unequipped gear worse than equipped first.
  - Equipment progression requirements preserved: best-in-slot auto-equip by `equipSlot + score(maxDamage+defBonus)` remains active; staged enhancement plan remains explicit in `docs/DECISIONS.md` (early safe accumulation/no risky spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`scroll + npc + resource + non-combat + rate budget`).
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=22`, `exp=2707`, `gold=349`, `code=200`.
  - Runtime continuity evidence: daemon remains continuous (`live-runner-daemon.sh` + `live-strategy-runner.js` active).
  - Next 30m KPI: `death delta=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, combined `wait_combat_start_rate_limit+wait_combat_start_cooldown<=30%`, and progression `exp +>=80` or `gold +>=12` with smoke `code=200`.
...
- [2026-03-15 08:26 KST] Hourly gameplay-feedback cycle (Buju API live probe).
  ...
  - Anomaly: telemetry split persists (`activity:fetch` health signal vs direct bearer-auth `401`), so this cycle is classified as `auth_or_source_mismatch` with low-confidence gameplay inference.
  - Dev feedback: hold gameplay-policy tuning; prioritize credential-path reconciliation and deterministic auth gating before consuming hourly metrics.
  - Failure mode + retry recommendation: on next cycle run masked preflight in one process (`/api/status` + `/api/logs?limit=1`), validate token scope/rotation in Buju dashboard, and only re-enable hourly KPI synthesis after both probes return `200`.
  - Next 30m actionable TODO: implement `scripts/auth-preflight-gate.js` to emit one machine-readable state (`ok|unauthorized|source_mismatch`) and short-circuit hourly summary generation when not `ok`.

- [2026-03-15 08:16 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` returned full window; deltas were `level +1` (`21->22`), `gold +25` (`329->354`), `inventory +2` (`7->9`), with low rate-limit signal (`2/20`) and no fresh death growth since latest thinking timestamp, so CHANGE was not applied.
  - Safety/efficiency policy kept with live evidence: `GET /api/status` => `200` (`level=22`, `exp=1473`, `gold=344`, `area=talking_island_field`) and current monster pool (`GET /api/areas/talking_island_field/monsters`) remains low-risk/high-efficiency (`rabbit`, `skeleton`); safest target remains `skeleton` under conservative risk-gap control.
  ...
  - Equipment progression requirements preserved: best-in-slot auto-equip by `equipSlot + score(maxDamage+defBonus)` remains active; staged enhancement plan in `docs/DECISIONS.md` remains explicit (early safe accumulation/no risky spam -> mid weapon-first after reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated and correctly safe-skips this cycle (`GET /api/npc/list => 200`, `npcs=[]`, inventory enchant scrolls absent).
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=22`, `exp=1475`, `gold=349`, `code=200`.
  ...
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-0817.json`); first attempt failed validation (`400 INVALID_INPUT`, `action_detail` length), corrected and reposted successfully.
  - Next 30m KPI: `death delta=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=30%`, and progression `exp +>=80` or `gold +>=12` with smoke `code=200`.

- [2026-03-15 07:46 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` returned full window; deltas were `level +2` (`20->22`), `gold +10` (`339->349`), `inventory +5` (`4->9`), with bounded rate-limit/throttle signal (`8/20`) and `recent death delta=0` since latest thinking timestamp, so CHANGE was not applied.
  - Safety/efficiency policy kept with live evidence: `GET /api/status` => `200` (`level=22`, `exp=1229`, `gold=354`, `area=talking_island_field`), and available monsters remain low-risk/high-efficiency (`rabbit`, `skeleton`), so safest-field hunt + strict threshold movement remains in force to avoid repeated defeats.
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation still prioritizes unequipped gear worse than equipped first.
  - Equipment progression preserved: best-in-slot auto-equip by `equipSlot + (maxDamage+defBonus)` remains active; staged enhancement policy in `docs/DECISIONS.md` remains (early safe accumulation/no risky spam -> mid weapon-first with reserve+prereqs -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (currently safe-skip when NPC/scroll prerequisites are unsatisfied).
  - Live evidence: smoke `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=22`, `exp=1223`, `gold=359`, `code=200`.
  - Runtime continuity evidence: daemon remains continuous (`ps -ax | grep -E "live-runner-daemon.sh|live-strategy-runner.js"` shows active daemon + runner).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}` (`tmp/thinking-post-0746.json`).
  - Next 30m KPI: `death delta=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=30%`, and progression `exp +>=80` or `gold +>=12` with smoke `code=200`.
...
- [2026-03-15 03:46 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Live evidence: `GET /api/status` + smoke run (`BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js`) => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=21`, `exp=3971`, `gold=324`, `code=200`; area monster pool remains safe (`rabbit`, `skeleton`).
  - Runtime continuity evidence: daemon remains continuous (`ps -ax | grep -E "live-runner-daemon.sh|live-strategy-runner.js"` shows active daemon + runner).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}`.
  - Next 30m KPI: `death delta=0`, inventory `<=8`, dangerous-surrender `<=1/8 cycles`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=30%`, and progression to `exp>=4050` or `gold>=340` with smoke `code=200`.
...
- [2026-03-14 20:46 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation still prioritizes unequipped gear worse than equipped first.
  - Equipment progression preserved: best-in-slot auto-equip by `equipSlot + (maxDamage+defBonus)` and staged enhancement policy retained; minimal safe enhancement path remains prerequisite-gated (`scroll+npc+resource+non-combat+rate budget`) and safely skipped this cycle (`GET /api/npc/list => 200`, `npcs=[]`).
  - Live evidence: `GET /api/status` => `200` (`level=21`, `exp=487`, `gold=324`, `area=talking_island_field`), `GET /api/areas/talking_island_field/monsters` confirms current safe pool (`rabbit`, `skeleton`), and smoke validation `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=21`, `exp=493`, `gold=339`, `code=200`.
  - Runtime continuity evidence: daemon remains continuous (`ps aux | grep -E "live-runner-daemon|live-strategy-runner"` shows active daemon + runner).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}`.
  - Next 30m KPI: `deaths<=3`, inventory `<=8`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=35%`, dangerous-surrender `<=1/8 cycles`, and progression to `exp>=560` or `gold>=350` with smoke `code=200`.
...
- [2026-03-14 19:16 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Hard constraints preserved exactly in runtime code: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation still prioritizes unequipped gear worse than equipped first.
  - Equipment progression requirements revalidated: best-in-slot evaluation/equip remains inventory-wide by `equipSlot + score(maxDamage+defBonus)`; staged enhancement plan remains documented in `docs/DECISIONS.md`; minimal safe enhancement path remains prerequisite-gated (`scroll + npc + resource + budget + non-combat`). Current probe: `GET /api/npc/list => 200` with `npcCount=0`, inventory enhancement scrolls unavailable, so enhancement is safely skipped.
  - Live evidence: smoke validation `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=combat_start`, `level=20`, `exp=3943`, `gold=344`, `code=200`.
  - Runtime continuity evidence: daemon remains continuous (`pgrep -fl "live-runner-daemon.sh|live-strategy-runner.js"` shows active daemon + runner).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}`.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=35%`, dangerous-surrender `<=1/8 cycles`, and progression to `exp>=3990` or `gold>=355` with smoke `code=200`.

- [2026-03-14 18:46 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` returned a full 20-log window (`2026-03-14 08:50:12 -> 2026-03-14 18:19:46`) with deltas `level +0` (`20->20`), `gold +10` (`309->319`), `inventory +5` (`3->8`), and persistent throttle/rate-limit wording across the window; improvement evidence (`gold +10`, no level regression) is present, so CHANGE was not applied.
  - Safety/efficiency evidence: active-area monster set (`GET /api/areas/talking_island_field/monsters`) remains low-risk/high-efficiency for current level gate (`rabbit`, `skeleton`), and strict level-threshold movement is preserved (`BUJU_MOVE_LEVEL_2=21`, current level 20).
  - Hard constraints preserved exactly in runtime code: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; when `slots>=10`, liquidation still prioritizes unequipped gear worse than equipped first.
  - Equipment progression requirements revalidated: best-in-slot evaluation/equip remains inventory-wide by `equipSlot + score(maxDamage+defBonus)`; staged enhancement plan is explicitly documented in `docs/DECISIONS.md` (early safe gold/no spam -> mid weapon-first on reserve threshold -> late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`scroll + npc + resource + budget + non-combat`). Current probe: `GET /api/npc/list => 200` with `npcCount=0`, inventory scrolls `[]`, so enhancement is safely skipped.
  - Live evidence: smoke validation `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=combat_start`, `level=20`, `exp=3707`, `gold=324`, `code=200`.
  - Runtime continuity evidence: daemon remains continuous (`ps` shows active `bash ./scripts/live-runner-daemon.sh` and `node scripts/live-strategy-runner.js`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}`.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=35%`, dangerous-surrender `<=1/8 cycles`, and progression to `exp>=3800` or `gold>=340` with smoke `code=200`.
...
- [2026-03-14 17:28 KST] Hourly gameplay-feedback cycle (Buju API live probe).
  ...
  - Anomalies: endpoints intended for direct hourly aggregation still degraded (`/api/activity/recent*` and `/api/logs/recent*` => `404`), and invalid probe contract still returns `400` (`/api/logs?action=combat&limit=200`, `limit>100`).
  - Dev feedback: keep status+paged-logs fallback as primary telemetry path; enforce valid log action enums and `limit<=100` in all probes to avoid avoidable 400 noise.
  - Failure mode + retry recommendation: for recurring `404` on `recent` routes, retry next cycle with capped backoff (e.g., 3 attempts, 2s/5s/10s) then continue with paged `/api/logs` fallback; do not block hourly summary.
  - Next 30m actionable TODO: add a small `hourly_log_sampler` helper that auto-pages `/api/logs` until 60-minute coverage and emits `wins/defeats/surrenders/potion_spend` as one structured JSON block for OPS ingestion.

- [2026-03-14 17:16 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` returned a full 20-log window (`2026-03-14 07:19:31 -> 2026-03-14 16:49:02`) with deltas `level +0` (`20->20`), `gold +100` (`309->409`), `inventory -1` (`3->2`), and throttle/rate-limit signals `8/20`; improvement evidence remains valid, so CHANGE was not applied.
  - Safety/efficiency evidence: confirmed available monsters in active area remain low-risk/high-efficiency (`rabbit`, `skeleton`) via `GET /api/areas/talking_island_field/monsters`; strict movement threshold gate retained (`BUJU_MOVE_LEVEL_2=21`, current level 20).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; slots>=10 still liquidate unequipped worse-than-equipped first.
  - Equipment progression preserved: best-in-slot auto-equip by `equipSlot + (maxDamage+defBonus)`; staged enhancement policy retained (early safe accumulation, mid weapon-first on reserve threshold, late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`scroll+npc+resource+rate+non-combat`).
  - Live evidence: smoke validation `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=combat_start`, `level=20`, `exp=2957`, `gold=319`, `code=200`.
  - Runtime continuity evidence: daemon remains continuous (`pgrep -fl "live-runner-daemon.sh|live-strategy-runner.js"` shows active daemon + runner).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}`.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=45%`, dangerous-surrender bursts `=0`, and progression to `exp>=3020` or `gold>=360` with smoke `code=200`.

- [2026-03-14 16:46 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  - KEEP (mandatory loop): `GET /api/agent/thinking/j211y?limit=20` returned a full 20-log window (`2026-03-14 06:49:30 -> 2026-03-14 16:20:33`) with positive deltas `level +1` (`19->20`), `gold +5` (`319->324`), `inventory -1` (`3->2`), and throttle/rate-limit signals `9/20`; improvement evidence remains valid, so CHANGE was not applied.
  - Safety/efficiency evidence: confirmed currently available monsters in active area are still low-risk/high-efficiency candidates (`rabbit`, `skeleton`) via `GET /api/areas/talking_island_field/monsters`; strict movement threshold gate retained (`BUJU_MOVE_LEVEL_2=21`, current level 20).
  - Hard constraints preserved exactly: `BUJU_INV_SELL_TRIGGER_SLOTS=10`, `BUJU_INV_SELL_TARGET_SLOTS=8`, `BUJU_INV_SELL_MAX_ITERATIONS_PER_TICK=10`; slots>=10 still liquidate unequipped worse-than-equipped first.
  - Equipment progression preserved: best-in-slot auto-equip by `equipSlot + (maxDamage+defBonus)`; staged enhancement policy retained (early safe accumulation, mid weapon-first on reserve threshold, late armor/accessory with failure-risk controls); minimal safe enhancement path remains prerequisite-gated (`scroll+npc+resource+rate+non-combat`).
  - Live evidence: smoke validation `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=20`, `exp=2717`, `gold=319`, `code=200`.
  - Runtime continuity evidence: daemon remains continuous (`pgrep -fl "live-runner-daemon.sh|live-strategy-runner.js"` shows active `live-runner-daemon.sh`).
  - Ops telemetry posted: `POST /api/agent/thinking` => `200 {"success":true}`.
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `wait_combat_start_rate_limit+wait_combat_start_cooldown<=45%`, dangerous-surrender bursts `=0`, and progression to `exp>=2780` or `gold>=350` with smoke `code=200`.
...
- [2026-03-14 11:46 KST] 30-min STRATEGY DIRECTOR run completed (adaptive mode + equipment progression).
  ...
  - Live evidence: smoke validation `BUJU_MAX_ACTIONS_PER_CYCLE=1 node scripts/live-strategy-runner.js` => `ok=1/1`, `lastAction=wait_combat_start_rate_limit`, `level=20`, `exp=247`, `gold=304`, `code=200`.
  - Runtime continuity evidence: daemon remains continuous (`pgrep -fl "live-runner-daemon.sh|live-strategy-runner.js"` shows active daemon + runner process).
  - Next 30m KPI: `deaths=0`, inventory `<=8`, `wait_combat_start_rate_limit<=30%`, and recovery to `gold>=315` or `exp>=300` with smoke `code=200`.
...
## [2026-03-16 11:30 KST] Hourly gameplay-feedback cycle
- Evidence (masked): loaded `BUJU_API_KEY` from `.env` in-process (raw key not printed), queried live `GET /api/status` and paged `GET /api/logs?page=n&limit=100` on `https://bujuagent.com` using the same `X-GQ-API-Key` header path.
- Last-hour gameplay signals (`60m`, `pages=5`, `events=496`): progression active via hunt throughput (`hunt=266`), outcomes `wins=266 / defeats=0` (observed defeat-rate `0.0%`), plus `surrender=11`.
- Resource trend: high consumable spend pressure (`buy=154`, `buy_spent=1540G`, `sell=3`, `sell_gain=210G`, `net_trade=-1330G`); status snapshot at probe time `level=24`, `exp=3929`, `gold=389`, `hp=217/445`.
- Anomalies: economy churn remains elevated (buy-heavy hour with low reserve), and surrender count is non-trivial despite perfect hunt-win observations.
...
## [2026-03-16 22:28 KST] Hourly gameplay-feedback cycle
...
- Probe outcome: both endpoints returned `401 UNAUTHORIZED` (`Missing or invalid API key`), so last-hour gameplay telemetry was not trustworthy this cycle.
- Collector cross-check: `npm run -s activity:fetch -- --hours 1` still observed `/api/status=200`, all configured `*/recent` endpoints `404`, and fallback zero-signal payload (`Δlevel=0`, `Δexp=0`, `Δgold=0`, `wins=0`, `defeats=0`).
- Last-hour gameplay signals (confidence: blocked): progression unknown, wins/defeats unknown, resource trend unknown due to auth/read-path inconsistency.
- Anomaly: split read/auth path persists (`direct status/logs=401` vs collector `/api/status=200`) with recent-history APIs unavailable (`404`).
...
## [2026-03-17 03:27 KST] Hourly gameplay-feedback cycle
- Evidence (masked): loaded `BUJU_API_KEY` from local `.env` in-process (`gq***80`, raw secret never printed), then queried live `GET /api/status` and `GET /api/logs?page=1&limit=100` on `https://bujuagent.com/api`.
- Probe outcome: canonical reads both failed with `401 UNAUTHORIZED` (`/api/status=401`, `/api/logs=401`), so trusted last-hour telemetry could not be collected this cycle.
- Last-hour gameplay signals (confidence: blocked): progression unknown, wins/defeats unknown, resource trend unknown (`events_in_window=0` from canonical logs path).
- Anomaly: repeated paired auth failure on status+logs in the same client/header path.
- Development feedback: freeze gameplay-policy tuning for this hour; prioritize auth-path recovery and preflight gating before KPI-driven strategy changes.
- Failure mode + retry recommendation: classify as `auth_blocked`; retry paired preflight (`/api/status`, `/api/logs?limit=1`) with jittered backoff (`10s`, `30s`, `60s`, max 3 attempts) and keep `inference_allowed=false` until both return `200` in one run.
- TODO (next 30-min): implement `auth-preflight-gate-v1` to persist `tmp/hourly-auth-preflight.json` with `{status_code_status,status_code_logs,auth_state,inference_allowed,retry_after_ms}` and hard-stop hourly KPI synthesis when `auth_state!=ok`.

## [2026-03-17 04:27 KST] Hourly gameplay-feedback cycle
...
- Probe outcome: canonical reads failed with `401 UNAUTHORIZED` (`/api/status=401`, `/api/logs=401`), and recent endpoint remains unavailable (`/api/logs/recent=404`).
- Last-hour gameplay signals (confidence: blocked): progression unknown, wins/defeats unknown, resource trend unknown (`events_in_window=0` from canonical logs path).
- Anomaly: recurring auth/read-path failure pattern persists (paired auth failure + missing recent endpoint).
...
2026-03-21 01:30:37 KST restarted scripts/live-runner-daemon.sh
- 2026-03-21 02:10:31 KST | hourly-buju-feedback | source=`fallback:local_replay` (`tmp/hourly-feedback-probe.json`) | last-hour signals: progression Δ(`level=0`,`exp=0`,`gold=0`), outcomes (`win=0`,`defeat=0`), action statuses (`success=0`,`failed=0`,`skipped=0`).
- 2026-03-21 02:10:31 KST | hourly-buju-feedback | live API probe status: `/api/status` observed via collector path (`200` in endpoint_statuses), while `recent/logs` family remained `404` across all probed variants (`/api/activity/recent*`, `/api/logs/recent*`, `/api/battle/logs/recent*`), causing zero-evidence fallback summary.
